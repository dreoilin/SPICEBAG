...
def ac_analysis(circ, start, points, stop, sweep_type=None,
                x0=None, M=None, D=None, ZAC=None, J=None,
                outfile="stdout"):
    ...
    if M is None:
        M0, _ = circ.generate_M0_and_ZDC()
        logging.debug("Reducing MNA matrix")
        M = M0[1:, 1:]
    if ZAC is None:
        Nac = _generate_Nac(circ)
        Nac = utilities.remove_row(Nac, rrow=0)
    if D is None:
        D0 = _generate_AC(circ, [M.shape[0], M.shape[0]])
        logging.debug("Reducing Dynamic matrix matrix")
        D = D0[1:, 1:]
        

    if circ.is_nonlinear():
        # linearised Jacobian provided
        if J is not None:
            pass
        else:
            if x0 is None:
            ...
            x0.print_short()
            logging.info("Linearizing the circuit...")
            J = _generate_J(xop=x0.asarray(), circ=circ,
                            reduced_mna_size=M.shape[0])
            logging.info(" done.")
    else:
        # linear circuit
        J = 0
